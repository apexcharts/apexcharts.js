 ...
 */
export function roundPathCorners(pathString, radius) {
...
export function drawCircle(ctx, radius, attrs = null) {
  const w = ctx.w
...
export function group(ctx, attrs = null) {
  const w = ctx.w
...
export function line(x, y, hORv = null) {
  let line = null
  if (hORv === null) {
    line = [' L', x, y].join(' ')
  } else if (hORv === 'H') {
...
export function curve(x1, y1, x2, y2, x, y) {
  let curve = ['C', x1, y1, x2, y2, x, y].join(' ')
  return curve
}

export function quadraticCurve(x1, y1, x, y) {
...
export function renderPaths(
  ...
  let dynamicAnim =
    initialAnim && w.config.chart.animations.dynamicAnimation.enabled
  ...
  if (w.config.chart.dropShadow.enabled && drawShadow) {
    Filters.dropShadow(ctx, el, w.config.chart.dropShadow, realIndex)
  }

  if (bindEventsOnPaths) {
    el.node.addEventListener('mouseenter', (e) => pathMouseEnter(ctx, el, e))
    el.node.addEventListener('mouseleave', (e) => pathMouseLeave(ctx, el, e))
    el.node.addEventListener('mousedown', (e) => pathMouseDown(ctx, el, e))
  }
  ...
  if (initialAnim && !w.globals.resized && !w.globals.dataChanged) {
    Animations.animatePathsGradually(ctx, {
      ...defaultAnimateOpts,
      speed: initialSpeed,
    })
  } else {
    if (w.globals.resized || !w.globals.dataChanged) {
      Animations.showDelayedElements(w)
    }
  }
...
export function getTextBasedOnMaxWidth(
  ctx,
  { text, maxWidth, fontSize, fontFamily }
) {
  const tRects = getTextRects(ctx, text, fontSize, fontFamily)
  ...
  if (Array.isArray(text)) {
    elText = w.globals.dom.Paper.text((add) => {
      for (let i = 0; i < text.length; i++) {
        ...
        if (maxWidth) {
          truncatedText = getTextBasedOnMaxWidth(ctx, {
            text: text[i],
            ...commonProps,
          })
        }
...
export function getMarkerPath(x, y, type, size) {
  let d = ''
  switch (type) {
    case 'cross':
      size = size / 1.4
      d = `M ${x - size} ${y - size} L ${x + size} ${y + size}  M ${x - size} ${
        y + size
      } L ${x + size} ${y - size}`
      break
    case 'plus':
 ...
 */
export function drawMarkerShape(ctx, x, y, type, size, opts) {
  const path = drawPath(ctx, {
    d: getMarkerPath(x, y, type, size, opts),
...
export function drawMarker(ctx, x, y, opts) {
  ...
  return drawMarkerShape(ctx, x, y, opts?.shape, size, {
    ...opts,
...
export function pathMouseEnter(ctx, path, e) {
  let w = ctx.w
  ...
  ctx.events.fireEvent('dataPointMouseEnter', [
    e,
    ctx,
  ...
  if (w.config.states.hover.filter.type !== 'none') {
    if (!w.globals.isTouchDevice) {
      ...
      Filters.applyFilter(ctx, path, i, hoverFilter.type)
    }
  }
}

export function pathMouseLeave(ctx, path, e) {
  let w = ctx.w
  ...
  ctx.events.fireEvent('dataPointMouseLeave', [
    e,
    ctx,
    { seriesIndex: i, dataPointIndex: j, w },
  ])
  ...
  if (w.config.states.hover.filter.type !== 'none') {
    Filters.getDefaultFilter(ctx, path, i)
  }
}

export function pathMouseDown(ctx, path, e) {
  let w = ctx.w
    ...
    if (
      ...
      const deSelect = (els) => {
        Array.prototype.forEach.call(els, (el) => {
          ...
          Filters.getDefaultFilter(ctx, el, i)
        })
      }
      deSelect(elPaths)
      deSelect(elCircles)
  ...
  if (selected === 'true') {
    ...
    if (activeFilter !== 'none') {
      Filters.applyFilter(ctx, path, i, activeFilter.type)
    } else {
...
export function rotateAroundCenter(el) {
  let coord = {}
 ...
 * @memberof Graphics
 **/
export function placeTextWithEllipsis(textObj, textString, width) {
