...
/**
 * ApexCharts Aster Class for drawing Aster Charts.
 * @module Aster
 **/
...
class Aster {
  ...
  draw(series) {
    ...
    for (let k = 0; k < series.length; k++) {
      // CALCULATE THE TOTAL
      total += Utils.negToZero(series[k][0])
    }
    ...
    series.forEach((m) => {
      this.maxY = Math.max(this.maxY, m[1])
    })
    ...
    for (let i = 0; i < series.length; i++) {
      ...
      let angle = this.fullAngle * (Utils.negToZero(series[i][0]) / total)
      this.sectorAngleArr.push(angle)

      // sectorAngleArr[i] = this.fullAngle / series.length
      this.sliceSizes.push((w.globals.radialSize * series[i][1]) / this.maxY)
    }
    ...
    let scaleSize = w.config.plotOptions.aster.customScale
    ...
    let elG = self.drawArcs(this.sectorAngleArr, series)
  ...
  drawArcs(sectorAngleArr, series) {
    ...
    for (let i = 0; i < sectorAngleArr.length; i++) {
      ...
      let elPath = graphics.drawPath({
        ...
        classes: `apexcharts-pie-area apexcharts-polararea-slice-${i}`,
      })
      ...
      labelPosition = Utils.polarToCartesian(
        ...
        w.globals.radialSize / 1.25 +
          w.config.plotOptions.aster.dataLabels.offset,
      ...
      if (w.config.dataLabels.enabled) {
        ...
        if (
          angle !== 0 &&
          w.config.plotOptions.aster.dataLabels.minAngleToShowLabel <
            sectorAngleArr[i]
        ) {
  ...
  pieClicked(i) {
    ...
    let size =
      me.sliceSizes[i] + (w.config.plotOptions.aster.expandOnClick ? 4 : 0)
    let elPath = w.globals.dom.Paper.select(`.apexcharts-polararea-slice-${i}`)
      .members[0]
  ...
  drawPolarElements(parent) {
    ...
    for (let i = 0; i < len - 1; i++) {
      ...
      circle.attr({
        ...
        cy: this.centerY,
        fill: 'none',
        'stroke-width': w.config.plotOptions.aster.rings.strokeWidth,
        stroke: w.config.plotOptions.aster.rings.strokeColor,
      })
  ...
  renderInnerDataLabels(dataLabelsConfig, opts) {
    ...
    let g = graphics.group({
      ...
      }) scale(${w.config.plotOptions.aster.customScale})`,
    })
    ...
    if (showTotal) {
      ...
      if (w.globals.series.length === 1) {
        val = lbFormatter(w.globals.series[0][0], w)
  ...
  drawSpokes(parent) {
    const w = this.w
    const graphics = new Graphics(this.ctx)
    const spokeConfig = w.config.plotOptions.aster.spokes
    ...
    for (let i = 0; i < w.globals.series.length; i++) {
      let spokeAngle = this.sectorAngleArr[i]

      if (i > 0) {
        spokeAngle += this.sectorAngleArr
          .slice(0, i)
        ...
        if (
          ...
          if (w.globals.selectedDataPoints[0].length > 0) {
            ...
            const el = w.globals.dom.baseEl.querySelector(
              `.apexcharts-polararea-slice-${index}`
            )
