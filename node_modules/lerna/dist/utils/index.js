...
function getPackageManifestPath(node, files) {
  const pkgJsonPath = (0, import_path5.resolve)((0, import_path5.join)(node.data.root, "package.json"));
  return files.find((f) => (0, import_path5.resolve)(f.file) === pkgJsonPath)?.file;
}
...
async function createProjectGraphWithPackages(projectGraph, projectFileMap, packageConfigs) {
  ...
  const tuples = await Promise.all(
    projectNodesMatchingPackageConfigs.map(
      (node) => new Promise((resolve3) => {
        ...
        if (manifestPath) {
          const fullManifestPath = (0, import_path6.join)(_workspaceRoot, manifestPath);
  ...
  sortedTuples.forEach(([node, manifest]) => {
    let pkg = null;
    if (manifest) {
      pkg = new Package(manifest, (0, import_path6.join)(_workspaceRoot, node.data.root), _workspaceRoot);
...
// libs/core/src/lib/command/index.ts
var DEFAULT_CONCURRENCY = import_os2.default.cpus().length;
...
var tempfile = (filePath) => import_path10.default.join(import_temp_dir.default, uuid.v4(), filePath || "");
...
async function detectProjects2() {
  const project = new Project(import_devkit5.workspaceRoot);
  return detectProjects(project.packageConfigs);
}
