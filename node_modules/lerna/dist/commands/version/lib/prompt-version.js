...
var prompt_version_exports = {};
__export(prompt_version_exports, {
  makePromptVersion: () => makePromptVersion
});
...
function promptVersion(currentVersion, name, prereleaseId, buildMetadata) {
  const patch = applyBuildMetadata(import_semver5.default.inc(currentVersion, "patch"), buildMetadata);
  const minor = applyBuildMetadata(import_semver5.default.inc(currentVersion, "minor"), buildMetadata);
  const major = applyBuildMetadata(import_semver5.default.inc(currentVersion, "major"), buildMetadata);
  const prepatch = applyBuildMetadata(import_semver5.default.inc(currentVersion, "prepatch", prereleaseId), buildMetadata);
  const preminor = applyBuildMetadata(import_semver5.default.inc(currentVersion, "preminor", prereleaseId), buildMetadata);
  const premajor = applyBuildMetadata(import_semver5.default.inc(currentVersion, "premajor", prereleaseId), buildMetadata);
  ...
  return promptSelectOne(message, {
    ...
    if (choice === "PRERELEASE") {
      ...
      return promptTextInput(`Enter a prerelease identifier ${prompt}`, {
        filter: (v) => applyBuildMetadata(import_semver5.default.inc(currentVersion, "prerelease", v || prereleaseId), buildMetadata)
      });
    }
    return choice;
  });
...
module.exports = (init_prompt_version(), __toCommonJS(prompt_version_exports));
